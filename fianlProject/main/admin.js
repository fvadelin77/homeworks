...
// cand state.idxEdit = -1, e in modul de adaugare produs
...
function populateData() {
  let table = document.querySelector("tbody");
  let str = "";
  for (let i = 0; i < adminState.db.length; i++) {
    let product = adminState.db[i];
    if (product === null) {
      continue;
    } else {
      str += `
      <tr>
        ...
        <td>${product.stoc}</td>    
        <td>
            <button class="btn px-2 py-1 editBtn" onclick="edit(${i})">
                <i class="bi bi-pencil-fill link-primary"></i>
            </button>
        </td>
        <td>
            <button class="btn px-2 py-1 deleteBtn" onclick="deleteItem(${i})">
                <i class="bi bi-x-circle-fill link-danger"></i>
            </button>
        </td>        
    </tr>
